...
% \\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\
%     Problem 3:  Plotting 2 intersecting planes (infinite # of solutions)
%
% \\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\


% -- Suppose we have an Ax = b equation, where A = (2 x 3), and it 
%    representes these 2 equations:
...
%%_solution = [ -10  10  5 ]';   %  <---  you need to change this !!

x_solution = [2/3  -2/3  0 ]'  + 0 .* [ 1  5  3 ]';


% -- Plot it as a black dot using the "plot3" command:

x_solution_plothandle = plot3( x_solution(1), x_solution(2), x_solution(3), '.');
set(x_solution_plothandle, 'Color', 'red', 'Markersize', 30);


% -- Now, let's plot 4 more vlack dots that respresent the intersection
%    of the 2 planes.

xp0 = [ 2/3  -2/3  0 ]';  % -- particular solution
xn0 = [ 1      5   3 ]';  % -- nullspace solutions

x_complete = [];
t =  [-1  -0.5  0.5  1 ]; % -- free variable values


for count = 1 : 1: length(t)

    % -- Calculate complete solutions 
    x_complete(:,count) = xp0 + t(count).*xn0

    % -- Plot it !!
    xcomp_plothandle(count) = plot3(x_complete(1,count), x_complete(2,count), x_complete(3,count), '.');
    set(xcomp_plothandle(count), 'Color', 'black', 'Markersize', 30);
...
ylabel('x2-axis');
zlabel('x3-axis');
title('Problem 2:  2 intersecting planes (inifnite # of solutions)');
...
% -- Then, add the 4th legend (the x = solution black dot)
 my_legend_strings{count+1}  = strcat( 'x_{free variable = 0} = [ ', 32, num2str(x_solution'), 32, ']^T'  );
